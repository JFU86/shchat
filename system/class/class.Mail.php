...
{	
		...
		if (isset($config["smtp"]["isActive"]) && $config["smtp"]["isActive"] == true){
			...
			if ($this->validate($receiver) && $this->validate($sender)){
				...
				if ($addHeader == true){
					...
					$msg = trim($msg);
					$msg = str_replace("ï»¿", "", $msg); // UTF8 BOM FIX
				}
	...
	{		
		...
		$mail->Password   = (!isset($config["smtp"]["encpw"]) || $config["smtp"]["encpw"]) ? $crypto->decrypt($config["smtp"]["password"],2) : $config["smtp"]["password"];    // SMTP account password
		$mail->SMTPSecure = (isset($config["smtp"]["useTLS"]) && $config["smtp"]["useTLS"]) ? "tls" : "";		
		return $mail;
	}
}
