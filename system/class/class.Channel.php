...
{	
	...
	{		
		...
		$channel = $this->getChannelName($channel_id);
	...
	{		
		global $db;
		$query = $this->query(
			"INSERT INTO {$db["channel"]} ".
			"VALUES ('', '{$channel_name}', 0, 0, 1, 1, 'Willkommen im Separee! Dieser Channel ist versteckt und wird geschlossen, sobald er leer ist.')"
		);
	}
	...
	{		
		global $db;
		$this->query("DELETE FROM {$db["online"]} WHERE username = '{$username}'");
	}
	...
	{		
		...
		while ($row = $this->fetchAssoc($result)) {
			...
			if ($row["username"] != "Chatbot" && $row["username"] != $user->getUserName()) {
				$userlist .= "<a href=\"javascript:whisper('{$row["username"]}');\"><img src='templates/{$config["template_name"]}/img/msg.gif' alt='whisper' /></a>";
			} else {
				$userlist .= "<img src='templates/{$config["template_name"]}/img/msg1.gif' alt='whisper' />";
			}			
		...
		if ($this->getUserLimit() == 0) {
			$output = "<div id=\"online-channel\">{$channel_name} <span id=\"online-users\">({$this->getOnlineUsers($this->getChannelName())})</span></div>{$userlist}";
		} else {
			$output = "<div id=\"online-channel\">{$channel_name} <span id=\"online-users\">({$this->getOnlineUsers($this->getChannelName())}/{$this->getUserLimit()})</span></div>{$userlist}";
		}
	...
	{		
		...
		if ($limited) {
			$result = $this->result("SELECT * FROM (SELECT id,channel FROM {$db["channel"]} WHERE id = 1) AS s1 UNION ALL SELECT * FROM (SELECT id,channel FROM {$db["channel"]} WHERE id > 1 AND hidden = 0 and minlevel <= {$level} ORDER BY channel) AS s2");
		} else {
	...
	{		
		...
		{		
			if (!$config["old_lines"]) {
				...
				$result = $this->result(
					"SELECT * FROM {$db["postings"]} WHERE `time` >= '{$gestern}' ".
					"AND (`channel` = '{$this->getChannelName()}' OR (`channel` = 'WHISPER' AND `whisperto` = '{$username}') ".
					"OR (`channel` = 'WHISPER' AND `username` = '{$username}') OR (`channel` = 'SYSTEM' AND `whisperto` = '{$username}') ".
					"OR `channel` = 'BROADCAST') ORDER BY id DESC LIMIT 50"
				);
			}
			...
			while ($row = $this->fetchAssoc($result)) {
				...
				if ($row["channel"] != "WHISPER" && $row["channel"] != "SYSTEM" && $row["channel"] != "BROADCAST") {
				...
				} elseif ($row["channel"] == "WHISPER" && $row["whisperto"] == $username) {
					$output .= "<span class=\"content-time\">{$time}</span> <span color=\"{$row["color"]}\"><a href=\"javascript:whisper('{$row["username"]}');\" style=\"color: {$row["color"]}\"><i>". sprintf($language->translate("<b>%s</b> flüstert"), $row["username"]) . "</a>: {$row["message"]}</i></span><br />\r\n";
				} elseif ($row["channel"] == "WHISPER" && $row["username"] == $username) {
					$output .= "<span class=\"content-time\">{$time}</span> <span color=\"{$row["color"]}\">". $language->translate("Ihr flüstert") ." <b><a href=\"javascript:whisper('{$row["whisperto"]}');\"> {$row["whisperto"]}</a></b>: <i>{$row["message"]}</i></span><br />\r\n";
					...
					$output .= "<span class=\"content-time\">{$time}</span> <span><i>". $language->translate("Broadcast von") ." <b>{$row["username"]}</b>: {$row["message"]}</font></i></span><br />\r\n";
				}
			...
			$output .= "<span><b>{$language->translate("SESSION ABGELAUFEN!")}</b></span><br />{$language->translate("Mögliche Ursachen: Du wurdest aus dem Chat gekickt oder warst zu lange inaktiv.")}";
			$output .= "<p><a href=\"index.php\" target=\"_self\">{$language->translate("Zur Startseite wechseln")}</a></p>";
	...
	{		
		...
		$user = new User();
		$check_time = time() - 60;
		$result = $this->result("SELECT username FROM {$db["user"]} WHERE last_reload <= {$check_time} AND status = 1");
		
		while ($row = $this->fetchAssoc($result)) {
			// Getrennte Benutzer löschen
			$this->removeUser($row["username"]);
			$query = $this->query("UPDATE {$db["user"]} SET status = 0 WHERE username = '{$row["username"]}'");
		...
		$result = $this->result("SELECT c.channel,count(o.id) as anzahl FROM {$db["channel"]} c LEFT JOIN {$db["online"]} o ON c.channel=o.channel WHERE c.hidden = 1 AND c.minlevel = 1 GROUP by o.id HAVING anzahl = 0");
		while ($row = $this->fetchAssoc($result)) {
			$query = $this->query("DELETE FROM {$db["channel"]} WHERE channel = '{$row["channel"]}'");
		}
	}
